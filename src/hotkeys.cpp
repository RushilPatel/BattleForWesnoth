/* $Id: hotkeys.cpp 55984 2013-01-01 09:34:55Z mordante $ */
/*
...
const struct {
	...
	{ hotkey::HOTKEY_REPLAY_SHOW_EVERYTHING, "replayshoweverything",
	  N_("Full Map (Replay Only!)"), false, hotkey::SCOPE_GAME },
...
void execute_command(display& disp, HOTKEY_COMMAND command, command_executor* executor, int index)
{
	...
	switch(command) {
		...
		case HOTKEY_ACCELERATED:
			...
			game_speed_label(preferences::turbo() , true);
...
//handles the on-screen game speed label
int handle_active;
void game_speed_label(bool state , bool show)
{
	std::string message = "Game Speed: 1x";
	//update gamespeed if turbo is enabled.
	if(state)
	{
		message = "Game Speed: " + preferences::get("turbo_speed") + "x";	
	}

	font::remove_floating_label(handle_active);//remove the old label
	handle_active = font::add_floating_label(font::floating_label(message));//re add the label with updated speed

	font::move_floating_label(handle_active,preferences::xwindow_size()-70.0, preferences::ywindow_size()-70.0); //bottom right corner
	//font::move_floating_label(handle_active,preferences::xwindow_size()-70.0, 3.0); //top right corner
	font:: show_floating_label(handle_active,show);
}
